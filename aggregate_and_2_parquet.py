...
list_files = [x.resolve() for x in input.glob('**/*.txt')]
chunks = np.array_split(list_files, 10)
for ix, chunk in enumerate(chunks):
    output_filename = output / f"chunk_{ix}.parquet"
    print(output_filename.resolve())
    tmp_df = pd.concat(
        (pd.read_table(filename, delimiter='\t', header=None, index_col=0).transpose() for filename in chunk),
        axis=0)
    tmp_df.set_index(pd.Series([Path(x).with_suffix('').stem for  x in chunk]), inplace=True)
    tmp_df.to_parquet(output_filename.resolve())
    del tmp_df

list_chunks = [x.resolve() for x in output.glob('**/*.parquet')]
