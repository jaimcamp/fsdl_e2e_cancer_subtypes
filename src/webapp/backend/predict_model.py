...
def _read_new_data(path):
    dataset = torch.tensor(
        pd.read_csv(BytesIO(path)).values
    )
...
def updateTSNE(modelpath, newdatapath, originaldatapath):
    model, optimizer = _load_model(modelpath)
    newdata = _read_new_data(newdatapath)
    mu_result, _ = _predict(model, optimizer, newdata)
    tsne_df_new = _clustering(mu_result)
    tsne_df_old = _read_old_tse(originaldatapath)
    tsne_df_cat = pd.concat([tsne_df_new, tsne_df_old])
    oldpath = Path(originaldatapath)
